...
def sliding_window_mmd_batch(
    ensemble_preds, window_size, kernel="rbf", step: int = 1, bandwidth_range: list[float] = None
) -> torch.Tensor:
    _, batch_size, seq_len = ensemble_preds.shape
    ...
    for i, (history_idx, future_idx) in enumerate(zip(idxs_list[:-1], idxs_list[1:])):
        ...
        mmd_batch = MMD_batch(X, Y, kernel=kernel, bandwidth_range=bandwidth_range)
...
def anchor_window_detector_batch(
    ...
    for future_idx in future_idx_range:
        ...
        for i in range(window_size): # TODO: fix iteration
            ...
            if distance == "mmd":
                dist_batch += MMD_batch(anchor, future, kernel=kernel, bandwidth_range=bandwidth_range)
            else:
